...
def get_args():
    ...
    parser.add_argument('--target_file', type=str, default='', help='file with pl labels on the test set')
...
test_masks=np.array(test['u_out']==0).reshape(-1, 80)
...
#test.drop(['pressure', 'id', 'breath_id'], axis=1, inplace=True)
...
test_targets=pd.read_csv(os.path.join(args.path,args.target_file))['pressure'].to_numpy().reshape(-1, 80)
...
train_dataset = SAKTDataset(test,test_targets,test_masks)
...
model.load_state_dict(torch.load(f'LSTM_models/model{args.fold}.pth'))

from Logger import *
model_dir=f'{args.pos_encode}_models'
log_dir=f'{args.pos_encode}_logs'
results_dir=f'{args.pos_encode}_val_results'

os.system(f'mkdir {log_dir}')
logger=CSVLogger(['epoch','train_loss','val_loss','val_mcmae'],f'{log_dir}/pl_log_fold{args.fold}.csv')
...
for epoch in range(args.epochs):
    ...
    if val_metric < best_metric:
        ...
        torch.save(model.state_dict(),f'{model_dir}/model{args.fold}_pl.pth')
