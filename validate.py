...
for i in range(len(columns)):
    results.append({'feature':columns[i],'mae':errors[i]})

# DISPLAY LSTM FEATURE IMPORTANCE
import matplotlib.pyplot as plt
#print()
baseline_mae=errors[-1]
df = pd.DataFrame(results)
df = df.sort_values('mae')
plt.figure(figsize=(10,20))
plt.barh(np.arange(len(columns)),df.mae)
plt.yticks(np.arange(len(columns)),df.feature.values)
plt.title('LSTM Feature Importance',size=16)
plt.ylim((-1,len(columns)))
plt.plot([baseline_mae,baseline_mae],[-1,len(columns)], '--', color='orange',
         label=f'Baseline OOF\nMAE={baseline_mae:.3f}')
plt.xlabel(f'Fold {args.fold+1} OOF MAE with feature permuted',size=14)
plt.ylabel('Feature',size=14)
plt.legend()
#plt.show()
os.system('mkdir feature_importance')
plt.savefig(f'feature_importance/fold{args.fold}.png',dpi=200)

# SAVE LSTM FEATURE IMPORTANCE
df = df.sort_values('mae',ascending=False)
df.to_csv(f'lstm_feature_importance_fold_{args.fold}.csv',index=False)


#errors=np.array(errors)
#errors=errors[:args.nfeatures]-errors[-1]
